...
import { useActiveSubscriptionQuery, useGetAllSubscriptionsQuery, useCancelSubscriptionMutation } from "@/redux/api/subscriptionApi/subscriptionApi";
import { usePathname } from "next/navigation";
import Loading from "@/components/Others/Loading";
import { useState } from "react";
import PassPayment from "./payment/PassPayment";
...
const SubscriptionPlan: React.FC = () => {
  ...
  const plans = getAllSubscriptions?.data || [];
  const activeStatus = activeSubcripiton?.data?.status;
  const activePlanId = String(activeSubcripiton?.data?.plan?.id || activeSubcripiton?.data?.planId || "");
  ...
  const handleCancelSubscription = async () => {
    try {
      const response = await cancelSubscription({ cancelAtPeriodEnd: false }).unwrap();
      if (response?.success) {
      ...
      {showCancelModal && (
        ...
        >
          ...
          >
            ...
            <h2 className="text-xl font-bold text-[#2D2D2D] mb-2">Cancel Subscription?</h2>
            <p className="text-center text-[#636F85] text-sm mb-7 leading-relaxed">
              Your subscription will be canceled immediately. You will lose access to all premium features right away.
            </p>
            <button
              onClick={handleCancelSubscription}
              disabled={isCanceling}
              className="w-full flex items-center justify-center gap-2 bg-red-600 hover:bg-red-700 disabled:opacity-60 text-white font-medium py-2 md:py-3.5 text-sm md:text-lg rounded-full transition cursor-pointer mb-3"
            >
              {isCanceling ? "Canceling..." : "Yes, Cancel Subscription"}
            </button>
            <button
              onClick={() => setShowCancelModal(false)}
              className="w-full flex items-center justify-center bg-[#F3F4F6] hover:bg-[#E5E7EB] text-[#2D2D2D] font-medium py-2 md:py-3.5 text-sm md:text-lg rounded-full transition cursor-pointer"
            >
        ...
        <div className="mb-8">
          ...
          {plans.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-20 px-4">
              <div className="w-16 h-16 rounded-full bg-[#F0ECFB] flex items-center justify-center mb-5">
                <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="#6E51E0" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                  <rect x="2" y="5" width="20" height="14" rx="2" />
                  <line x1="2" y1="10" x2="22" y2="10" />
                </svg>
              </div>
              <h3 className="text-xl font-semibold text-[#2D2D2D] mb-2">
                No {planType === "monthly" ? "Monthly" : "Yearly"} Plans Available
              </h3>
              <p className="text-[#636F85] text-sm text-center max-w-sm">
                There are no {planType} subscription plans at the moment.
                {planType === "monthly"
                  ? " Try switching to Yearly plans — you might find a great deal there."
                  : " Try switching to Monthly plans to see available options."}
              </p>
              <button
                onClick={togglePlan}
                className="mt-5 px-6 py-2.5 bg-[#6E51E0] text-white text-sm font-medium rounded-full hover:bg-[#5a3fd4] transition cursor-pointer"
              >
                Switch to {planType === "monthly" ? "Yearly" : "Monthly"}
              </button>
            </div>
          ) : (
            ...
            `}>
              {plans.map((plan: Plan, index: number) => {
                const isCurrentPlan = plan.id === String(activeSubcripiton?.data?.plan?.id);

                return (
                  <div
                    key={index}
                    className={`group flex h-full flex-col rounded-lg bg-white p-6 shadow-sm w-full ${plan.highlighted ? "ring-2 ring-indigo-600" : ""}`}
                  >
                    <div className="mb-4 rounded-xl bg-gray-2 p-4 bg-gray-100">
                      <div className="flex justify-between items-center">
                        <div
                          className={`mb-4 inline-block rounded-full px-4 py-1 text-sm font-medium transition-colors bg-white text-primaryBgColor hover:bg-primaryBgColor hover:text-white group-hover:bg-primaryBgColor group-hover:text-white`}
                        >
                          {plan.name}
                        </div>
                        <p className="text-sm text-indigo-600">
                      ...
                      <ul className="space-y-2.5">
                        {plan.features.map((feature: string, idx: number) => (
                          <li key={idx} className="flex items-start gap-2 text-sm text-gray-600">
                            <svg className="mt-0.5 h-7 w-7 flex-shrink-0 text-green-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <div className="flex flex-col items-center gap-3 w-full">
                      <button
                        ...
                        className={`w-full py-2 border text-sm font-medium rounded-full flex items-center justify-center gap-1.5 transition-colors
                          ${isCurrentPlan && activeSubcripiton?.data?.status === "trialing"
                            ? "cursor-not-allowed bg-amber-50 border-amber-200 text-amber-900"
                            : isCurrentPlan && activeSubcripiton?.data?.status === "active"
                              ? "cursor-not-allowed bg-green-50 border-green-200 text-green-800"
                              : "bg-[#FBFBFB] border-gray-200 text-[#2D2D2D] hover:bg-primaryBgColor hover:text-white group-hover:bg-primaryBgColor group-hover:text-white cursor-pointer"
                          }`}
                      ...
                      >
                        {isCurrentPlan && activeSubcripiton?.data?.status === "trialing" ? (
                          <>
                            Trial active
                            <span className="text-[11px] bg-amber-200 text-amber-900 px-2 py-0.5 rounded-full">
                              {Math.ceil(
                                (new Date(activeSubcripiton?.data?.currentPeriodEnd).getTime() - new Date().getTime())
                                / (1000 * 60 * 60 * 24)
                              )} days left
                            </span>
                          </>
                      ...
                      {/* Cancel Subscription — show for any plan that has an active subscription */}
                      {pathName === "/dashboard/subscriptions" && activeSubcripiton?.data && isCurrentPlan && (
                        <button
                          onClick={() => setShowCancelModal(true)}
                          className="w-full py-2 border border-red-200 bg-red-50 text-red-600 text-sm font-medium rounded-full flex items-center justify-center gap-1.5 hover:bg-red-600 hover:text-white transition-colors cursor-pointer"
                        >
                          <svg className="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
